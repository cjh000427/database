...
SELECT 
    ...
    (
        ...
        FROM locations loc
        WHERE loc.location_id = d.department_id
    ) AS 로케이션아이디,
    d.department_name,
    (
...
-- 오류 이유 : WHERE loc.country_id = c.country_id 
-- locations의 country_id 가 중복되는 값들이 있기 때문에 여러 값이 리턴됨.
-- 그러므로 스칼라 서브쿼리에 작성하면 오류가 발생함.
-- 해결 : FROM 절을 locations로 잡고 해야 함.
-----------------------------------------
